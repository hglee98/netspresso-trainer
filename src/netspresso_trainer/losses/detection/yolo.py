...
class YOLOFastestLoss(nn.Module):
    ...
    def forward(self, out: Dict, target: Dict) -> torch.Tensor:
        ...
        self.anchors = out["anchors"]
        out = out['pred']
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        if out[0].device != self.anchors.device:
            self.anchors = self.anchors.to(out[0].device)
        ...
        for k, o in enumerate(out):
            b, _, h, w, n_ch = o.shape
            stride_this_level = img_size // h
        ...
        for idx, o in enumerate(out):
            reg_output = o[..., :4]
            reg_output[..., 2:4] += torch.log(self.anchors[idx].clone())
    ...
    def get_l1_target(self, l1_target, gt, stride, x_shifts, y_shifts, eps=1e-8):
        ...
        l1_target[:, 2] = torch.log(gt[:, 2] + eps)
        l1_target[:, 3] = torch.log(gt[:, 3] + eps)
        return l1_target
    ...
    def get_losses(
        ...
        labels = torch.zeros(len(target), max_samples, 5).to(outputs.device)
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        grid = self.grids[k]
        batch_size, _, hsize, wsize, n_ch = output.shape
        if grid.shape[2:4] != output.shape[2:4]:
            yv, xv = torch.meshgrid(torch.arange(hsize), torch.arange(wsize), indexing="ij")
            grid = torch.stack((xv, yv), 2).view(1, 1, hsize, wsize, 2).repeat(1, 3, 1, 1, 1).type(dtype)
            self.grids[k] = grid
        output = torch.cat([
            (output[..., :2] + grid) * stride,
            torch.exp(output[..., 2:4]) * self.anchors[k],
            output[..., 4:]
        ], dim=-1).reshape(batch_size, -1, n_ch)
