...
https://github.com/lyuwenyu/RT-DETR/blob/main/rtdetr_pytorch/src/zoo/rtdetr/rtdetr_decoder.py
"""
...
class RTDETRTransformer(nn.Module):
    def __init__(
        ...
        assert len(feat_channels) <= self.num_levels
        assert len(self.feat_strides) == len(feat_channels)
        for _ in range(self.num_levels - len(self.feat_strides)):
        ...
        # Transformer module
        decoder_layer = TransformerDecoderLayer(self.hidden_dim, self.nhead, dim_feedforward, dropout, activation, self.num_levels, num_decoder_points)
    ...
    def _get_decoder_input(self,
        ...
        input_max, _ = torch.max(enc_outputs_class, dim=-1)
        _, topk_ind = torch.topk(input_max, self.num_queries, dim=1)
